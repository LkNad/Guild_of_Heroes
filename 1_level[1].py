...
FPS = 50
JUMP_POWER = 50  # сила с которой прыгает персонаж
...
wall_up_group = pygame.sprite.Group()
...
class Tile(pygame.sprite.Sprite):
    def __init__(self, tile_type, pos_x, pos_y):
        if tile_type == 'wall':
            ...
            super().__init__(tiles_group, all_sprites, wall_up_group)
        else:
...
class Player(pygame.sprite.Sprite):
    ...
    def jump(self):
        ...
        if (pygame.sprite.groupcollide(player_group, wall_group, False,
            ...
            # если игрок пересёк стену или границу мира, то прыжок отменяется
            self.rect.y -= JUMP_POWER
        else:
            # плавное падение после прыжка
            self.update_jump()


    def update(self):
        while not(pygame.sprite.groupcollide(wall_group, player_group, False,
                                       False) or not(
                pygame.sprite.groupcollide(tiles_group, player_group, False, False))):
            self.rect.y += 5 * GRAVITY
            all_sprites.update()
            ...
            pygame.display.flip()
            # обновил экран чтобы падение не было как 1 кадр
            clock.tick(FPS * 1.2)
        self.rect.y = self.rect.y //10 * 10
        clock.tick(FPS * 5)
...
player = Player(0, 2.2)  # Спавн игрока
camera = Camera()
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_LEFT:
                if pygame.key.get_mods() in [4097, 12289]:
                    ...
                    if pygame.sprite.groupcollide(player_group, wall_group,
                        ...
                        player.rect.x -= STEP
                            ...
                            pygame.sprite.groupcollide(tiles_group,
                                                       ...
                                                       False)):
                        player.rect.x -= STEP * 2
            if event.key == pygame.K_UP or event.key == pygame.K_SPACE and pygame.K_SPACE:
                if pygame.sprite.groupcollide(player_group, wall_group,
                                              False,
                                              False) or not (
                        pygame.sprite.groupcollide(tiles_group,
